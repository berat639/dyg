...
function init() {
  ...
  for (let i = 0; i < particleSystems.length; i++) {
    ...
    loadert.load('./assets/models/helvetiker_regular.typeface.js', function (res) { 
      const lines = [
        'Duygu cumartesi sabah',
        'seninle kahve',
        'keyifli olurdu'
      ];
      
      const textGroup = new THREE.Group();
      const lineHeight = window.innerWidth < 768 ? 200 : 90; // Smaller line height on mobile
      const fontSize = window.innerWidth < 768 ? 120 : 80;   // Smaller font size on mobile
      ...
      lines.forEach((line, index) => {
        ...
        const cubeMat = new THREE.MeshLambertMaterial({color: 0x2A4334})
        const textMesh = new THREE.Mesh(textGeo, cubeMat);
        textMesh.position.y = totalHeight/2 - (index * lineHeight);
        textGroup.add(textMesh);
      });
      
      textGroup.position.set(0, -200, 0);
      
      const updateTextScale = () => {
        const baseScale = Math.min(window.innerWidth / 1920, window.innerHeight / 1080);
        const mobileScale = window.innerWidth < 768 ? 2 : 1; // Increase scale on mobile
        textGroup.scale.set(baseScale * mobileScale, baseScale * mobileScale, baseScale * mobileScale);
      };
...
var materialLogo = new THREE.MeshBasicMaterial({
  map: loaderImage.load('./assets/models/logo2.png'),
  transparent: true,
  alphaTest: 0.5
});
var geometryLogo = new THREE.PlaneGeometry(14, 18);

var meshLogo = new THREE.Mesh(geometryLogo, materialLogo);
meshLogo.scale.normalize().multiplyScalar(0.3); 
meshLogo.position.set(0,280,260)
...
function createParticleSystems() {
  ...
  for (let i = 0; i < 100; i++) {
    const x = Math.random() * 2000 - 1300; // generate random number between -1000 to 1000
    const y = Math.random() * 2000 - 1300;
    const z = Math.random() * 2000 - 1300;
